...
onMounted(() => {
  ...
  function updateFrame() {
    ...
    step(dt);
    render(null);
    if (performance.now() - lastInputAt < 1400 || pointers[0].down) {
  ...
  function listen(target: Window | HTMLElement, type: string, handler: EventListener) {
  ...
  function handlePointerDown(event: PointerEvent) {
    ...
    updatePointerDownData(pointer, event.pointerId, scaleByPixelRatio(position.x), scaleByPixelRatio(position.y));
    clickSplat(pointer);
    startFrame();
  }
  ...
  function handlePointerMove(event: PointerEvent) {
    ...
    if (!position) return;
    const pointer = pointers[0];
    const color = pointer.color.r || pointer.color.g || pointer.color.b ? pointer.color : generateColor();
    updatePointerMoveData(pointer, scaleByPixelRatio(position.x), scaleByPixelRatio(position.y), color);
    if (pointer.moved) startFrame();
  }
  ...
  listen(window, "pointerdown", handlePointerDown as EventListener);
  listen(window, "pointermove", handlePointerMove as EventListener);
  listen(window, "pointerup", handlePointerUp as EventListener);
...
onBeforeUnmount(() => disposeFluidCursor());
</script>
...
<style scoped>
.fluid-cursor-root { position: absolute; inset: 0; z-index: 2; width: 100%; height: 100%; pointer-events: none; }
.fluid-cursor-canvas { display: block; width: 100%; height: 100%; pointer-events: none; }
</style>
