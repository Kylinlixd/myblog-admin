...
describe('Profile mounted interactions', () => {
  ...
  it('uses the two-column identity and form layout at the card level', async () => {
    ...
    expect(wrapper.find('.password-identity').exists()).toBe(false)
    expect(wrapper.find('.password-group--current').exists()).toBe(true)
    expect(wrapper.find('.password-group--new').exists()).toBe(true)
    expect(wrapper.find('.password-reset-button').exists()).toBe(true)
    expect(wrapper.find('.profile-form-stackable[layout="vertical"]').exists()).toBe(true)
    expect(wrapper.find('.profile-actions button').text()).toContain('撤销修改')
    expect(wrapper.findAll('.profile-actions button').at(-1).text()).toContain('保存资料')
