...
function startParticleTitle() {
  ...
  function resize() {
    ...
    const fontSize = Math.min(92, Math.max(42, width * 0.075)); const titleWidth = fontSize * 8.8; const startX = Math.max(18, (width - titleWidth) / 2); const startY = Math.max(70, height * 0.36)
...
.hero-title { display: flex; min-height: clamp(90px, 10vw, 120px); align-items: center; justify-content: center; gap: .18em; margin: 0; font-size: clamp(42px, 7.5vw, 92px); font-weight: 800; letter-spacing: -.065em; line-height: 1.08; text-align: center; white-space: nowrap; opacity: 0; transform: translateY(5px); transition: opacity .45s ease, transform .45s ease; }.hero-title--ready { opacity: 1; transform: translateY(0); }.hero-title__accent { color: var(--home-accent); }
