<template>
  <div class="home-page">
    <section class="home-hero" aria-labelledby="home-title">
      ...
      <div class="hero-glow-field" aria-hidden="true"><span class="hero-light-ribbon hero-light-ribbon--one" /><span class="hero-light-ribbon hero-light-ribbon--two" /><span class="hero-light-ribbon hero-light-ribbon--three" /><span class="hero-glow hero-glow--one" /><span class="hero-glow hero-glow--two" /><span class="hero-glow hero-glow--three" /></div>
...
function startParticleTitle() {
  ...
  function resize() {
    ...
    ambient = Array.from({ length: 14 }, (_, index) => ({ x: width * (0.58 + ((index * 19) % 34) / 100), y: height * (0.16 + ((index * 31) % 65) / 100), radius: 1.2 + (index % 3) * .65, alpha: 0.14 + (index % 3) * 0.025, phase: index * 1.9 }))
    network = [
      { x: width * .64, y: height * .22, width: width * .28, height: height * .1, alpha: .13, phase: .4 },
      { x: width * .71, y: height * .5, width: width * .22, height: -height * .08, alpha: .11, phase: 2.1 },
      { x: width * .59, y: height * .7, width: width * .32, height: -height * .12, alpha: .1, phase: 4.3 }
    ]
  ...
  function render(now) {
    ...
    for (const path of network) {
      ...
      context.beginPath(); context.arc(pulseX, pulseY, 2.2, 0, Math.PI * 2); context.fillStyle = `rgb(198 133 79 / ${path.alpha * 2.6})`; context.shadowBlur = 10; context.shadowColor = 'rgb(198 133 79 / 28%)'; context.fill(); context.shadowBlur = 0
    }
...
.home-hero::before { position: absolute; z-index: -2; inset: 5% 2% 7% 38%; border-radius: 42% 58% 48% 52%; background: radial-gradient(ellipse at 62% 34%, rgb(211 168 116 / 24%), transparent 32%), radial-gradient(ellipse at 72% 64%, rgb(139 157 132 / 22%), transparent 38%), radial-gradient(ellipse at 42% 52%, rgb(255 251 241 / 46%), transparent 64%); content: ''; filter: blur(26px); animation: hero-nebula 15s ease-in-out infinite alternate; }
.home-hero::after { position: absolute; z-index: -3; inset: 0; background: linear-gradient(112deg, #f6efe3 0%, #f7f0e6 38%, #f5eee2 65%, #f1ebdf 100%); content: ''; }
...
.hero-glow-field { position: absolute; z-index: -1; inset: 0; overflow: hidden; pointer-events: none; }.hero-glow-field::before, .hero-glow-field::after { position: absolute; display: block; width: 70%; height: 34%; border: 1px solid rgb(184 94 45 / 12%); border-right-color: transparent; border-left-color: transparent; border-radius: 50%; content: ''; filter: blur(.4px); transform: rotate(-12deg); animation: hero-orbit 13s ease-in-out infinite alternate; }.hero-glow-field::before { top: 12%; right: -7%; }.hero-glow-field::after { right: -2%; bottom: 11%; border-color: rgb(104 126 106 / 12%); border-left-color: transparent; animation-delay: -6s; }.hero-light-ribbon { position: absolute; display: block; width: 72%; height: 16%; border-top: 1px solid rgb(207 157 104 / 22%); border-radius: 50%; filter: blur(.6px); opacity: .85; transform: rotate(-15deg); animation: hero-ribbon-flow 12s ease-in-out infinite alternate; }.hero-light-ribbon::after { position: absolute; top: -32px; left: 12%; width: 72%; height: 64px; border-radius: 50%; background: linear-gradient(90deg, transparent, rgb(218 175 122 / 14%), rgb(130 151 132 / 11%), transparent); content: ''; filter: blur(13px); }.hero-light-ribbon--one { top: 18%; right: -8%; }.hero-light-ribbon--two { top: 42%; right: 2%; border-color: rgb(127 150 130 / 17%); animation-delay: -4s; transform: rotate(-8deg) scale(.82); }.hero-light-ribbon--two::after { background: linear-gradient(90deg, transparent, rgb(132 155 134 / 14%), transparent); }.hero-light-ribbon--three { right: -4%; bottom: 15%; border-color: rgb(194 137 86 / 16%); animation-delay: -8s; transform: rotate(-20deg) scale(.7); }.hero-glow { position: absolute; display: block; border-radius: 50%; filter: blur(2px); opacity: .7; animation: hero-glow-drift 7s ease-in-out infinite alternate; }.hero-glow::before { position: absolute; width: 180px; height: 120px; border-radius: 48% 52% 58% 42%; background: radial-gradient(ellipse, rgb(216 163 110 / 18%), transparent 70%); content: ''; filter: blur(20px); transform: translate(-50%, -50%); animation: hero-aura 8s ease-in-out infinite alternate; }.hero-glow--one { top: 16%; right: 15%; width: 10px; height: 10px; background: #d8a36e; box-shadow: 0 0 34px 14px rgb(216 163 110 / 22%); }.hero-glow--two { top: 29%; right: 27%; width: 7px; height: 7px; background: #9aa78f; box-shadow: 0 0 28px 10px rgb(154 167 143 / 20%); animation-delay: -2s; }.hero-glow--three { right: 7%; bottom: 22%; width: 9px; height: 9px; background: #d6b895; box-shadow: 0 0 30px 12px rgb(214 184 149 / 17%); animation-delay: -4s; }
...
@keyframes hero-orbit { from { transform: translate3d(-2%, 0, 0) rotate(-17deg) scale(.98); opacity: .55; } to { transform: translate3d(3%, 4%, 0) rotate(-12deg) scale(1.04); opacity: 1; } }
...
@keyframes hero-ribbon-flow { from { opacity: .34; transform: translate3d(-2%, 3%, 0) rotate(-15deg) scale(.94); } to { opacity: .9; transform: translate3d(4%, -4%, 0) rotate(-10deg) scale(1.06); } }
