    ...
    <section class="home-hero app-container" aria-labelledby="home-title">
      ...
      <div class="hero-copy">
        ...
        <div class="hero-actions hero-actions--centered">
          ...
          <a class="hero-button hero-button--secondary" href="#topics">按主题浏览</a>
        </div>
      </div>
...
function startParticleTitle() {
  ...
  function resize() {
    ...
    const fontSize = Math.min(92, Math.max(42, width * 0.075)); const titleWidth = fontSize * 4.35; const startX = Math.max(18, (width - titleWidth) / 2); const startY = Math.max(70, height * 0.3)
  ...
  function render(now) {
    ...
    for (const line of ambient) {
      ...
      context.fillStyle = `rgb(184 94 45 / ${line.alpha * 1.7})`; context.fillRect(line.x - 2, line.y - 2, 4, 4)
    }
...
.hero-copy { position: relative; z-index: 1; display: flex; max-width: 980px; flex-direction: column; align-items: center; padding: 24px; }
.hero-title { display: flex; min-height: clamp(116px, 14vw, 176px); flex-direction: column; justify-content: center; margin: 0; font-size: clamp(48px, 7.5vw, 92px); font-weight: 800; letter-spacing: -.065em; line-height: 1.08; text-align: center; opacity: 0; transform: translateY(5px); transition: opacity .45s ease, transform .45s ease; }.hero-title--ready { opacity: 1; transform: translateY(0); }.hero-title__accent { color: var(--home-accent); }
.hero-actions { display: flex; flex-wrap: wrap; align-items: center; gap: 10px; }.hero-actions--centered { justify-content: center; }
