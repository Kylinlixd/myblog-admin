...
function startParticleTitle() {
  ...
  function resize() {
    ...
    ambient = Array.from({ length: 22 }, (_, index) => ({ x: width * (0.56 + ((index * 17) % 37) / 100), y: height * (0.12 + ((index * 29) % 72) / 100), length: 42 + (index % 4) * 22, angle: -0.35 + (index % 5) * 0.18, alpha: 0.1 + (index % 3) * 0.025, phase: index * 1.7 }))
  ...
  function render(now) {
    ...
    for (const line of ambient) {
      const drift = reduceMotion ? 0 : Math.sin(now / 5200 + line.phase) * 10; const x = line.x + drift; const y = line.y + Math.cos(now / 6400 + line.phase) * 7; const tailX = x + Math.cos(line.angle) * line.length; const tailY = y + Math.sin(line.angle) * line.length
      context.beginPath(); context.moveTo(x, y); context.quadraticCurveTo(x + line.length * .45, y - 18, tailX, tailY); context.strokeStyle = `rgb(112 125 111 / ${line.alpha})`; context.stroke()
      context.beginPath(); context.arc(x, y, 1.8, 0, Math.PI * 2); context.fillStyle = `rgb(196 126 76 / ${line.alpha * 2})`; context.fill()
    }
...
.home-hero::before { position: absolute; z-index: -2; inset: 7% 5% 10% 42%; border-radius: 48% 36% 42% 30%; background: radial-gradient(circle at 56% 48%, rgb(211 187 149 / 25%), transparent 68%); content: ''; filter: blur(18px); animation: hero-haze 12s ease-in-out infinite alternate; }
.home-hero::after { position: absolute; z-index: -3; inset: 0; background: radial-gradient(ellipse at 75% 18%, rgb(219 177 126 / 20%), transparent 30%), radial-gradient(ellipse at 88% 74%, rgb(154 171 151 / 17%), transparent 34%), linear-gradient(112deg, transparent 35%, rgb(255 250 241 / 42%) 52%, transparent 72%); background-size: 120% 120%, 125% 125%, 180% 180%; content: ''; animation: hero-haze 16s ease-in-out infinite alternate-reverse; }
...
.hero-glow-field { position: absolute; z-index: -1; inset: 0; overflow: hidden; pointer-events: none; }.hero-glow-field::before, .hero-glow-field::after { position: absolute; display: block; width: 52%; height: 38%; border: 1px solid rgb(184 94 45 / 13%); border-radius: 50%; content: ''; filter: blur(.2px); transform: rotate(-17deg); animation: hero-orbit 11s ease-in-out infinite alternate; }.hero-glow-field::before { top: 5%; right: 1%; }.hero-glow-field::after { right: 11%; bottom: 3%; border-color: rgb(104 126 106 / 14%); animation-delay: -5s; }.hero-glow { position: absolute; display: block; border-radius: 50%; filter: blur(2px); opacity: .7; animation: hero-glow-drift 6s ease-in-out infinite alternate; }.hero-glow::before { position: absolute; width: 180px; height: 120px; border-radius: 48% 52% 58% 42%; background: radial-gradient(ellipse, rgb(216 163 110 / 18%), transparent 70%); content: ''; filter: blur(20px); transform: translate(-50%, -50%); animation: hero-aura 7s ease-in-out infinite alternate; }.hero-glow--one { top: 16%; right: 15%; width: 18px; height: 18px; background: #d8a36e; box-shadow: 0 0 38px 16px rgb(216 163 110 / 22%); }.hero-glow--two { top: 29%; right: 27%; width: 8px; height: 8px; background: #9aa78f; box-shadow: 0 0 30px 11px rgb(154 167 143 / 22%); animation-delay: -2s; }.hero-glow--three { right: 7%; bottom: 22%; width: 12px; height: 12px; background: #d6b895; box-shadow: 0 0 34px 14px rgb(214 184 149 / 18%); animation-delay: -4s; }
...
@keyframes hero-haze { from { background-position: 0% 0%, 100% 100%, 0% 50%; opacity: .62; } to { background-position: 10% 7%, 90% 93%, 100% 50%; opacity: 1; } }
