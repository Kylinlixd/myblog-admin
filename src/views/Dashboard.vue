...
async function acknowledgeCommentCount() {
  try {
    await commentNotifications.markRead({ latestCommentId: commentNotifications.latestCommentId })
  } catch (reason) {
    console.error('确认评论已读失败:', reason)
  }
}
...
@media (max-width: 1180px) { .operations-grid { grid-template-columns: 1fr; } .operations-side { grid-template-columns: 1fr; } }
    @media (max-width: 820px) { .dashboard-intro { grid-template-columns: 1fr; align-items: start; } .intro-actions { min-width: 0; justify-items: start; } .metric-rail { grid-template-columns: repeat(4, minmax(0, 1fr)); } .metric-item { min-height: 0; gap: 5px; padding: 14px 6px; border-right: 1px solid var(--color-border); border-bottom: 0; } .metric-item:nth-child(2) { border-right: 1px solid var(--color-border); } .metric-item:nth-child(-n + 2) { border-bottom: 0; } .metric-item:last-child { border-right: 0; } .metric-icon { font-size: 15px; } .metric-label { font-size: 10px; } .metric-value { font-size: 22px; } .operations-side { grid-template-columns: 1fr; } }
    @media (max-width: 640px) { .dashboard-intro { min-height: 0; padding: 26px 22px; border-radius: 16px 16px 7px 7px; } .intro-copy h1 { font-size: 34px; } .intro-actions { width: 100%; } .create-button { justify-content: center; } .metric-rail { grid-template-columns: repeat(4, minmax(0, 1fr)); } .metric-item, .metric-item:nth-child(2) { min-height: 0; gap: 4px; padding: 12px 4px; border-right: 1px solid var(--color-border); border-bottom: 0; } .metric-item:nth-child(4) { border-right: 0; } .metric-icon { font-size: 14px; } .metric-label { font-size: 9px; white-space: nowrap; } .metric-value-wrap { gap: 4px; } .metric-value { font-size: 18px; } .access-overview { grid-template-columns: 1fr 1fr; gap: 12px; } .access-overview a { grid-column: 1 / -1; padding-top: 8px; } .content-pulse, .taxonomy-panel { padding: 20px; } .panel-heading { align-items: flex-start; flex-direction: column; gap: 8px; } .trend-summary { grid-template-columns: 1fr; } .trend-summary div, .trend-summary div + div { padding: 13px 0; border-left: 0; border-bottom: 1px solid var(--color-border); } .trend-summary div:last-child { border-bottom: 0; } .trend-line-chart { min-height: 220px; margin-inline: -8px; padding-top: 18px; } }
