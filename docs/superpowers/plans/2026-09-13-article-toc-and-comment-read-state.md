...
- [x] 前端目录使用 `IntersectionObserver`，并在页面更新、路由切换和卸载时清理观察器。
- [x] 前端通知状态改为消费 `latest_comment_id`，移除 localStorage 评论数量基线。
- [x] 后端新增 `CommentReadState` 与 `0007_commentreadstate`，保留旧收据接口兼容。
- [x] 后端游标更新具备单调推进和当前可见最大 ID 上限，避免跳过未来评论。
- [x] 前端 CI 构建部署成功（commit `dbd249d`）。
- [x] 后端 CI 全量测试通过（commit `9eb73a0`）。
- [x] 生产数据库完成备份并执行 `comment.0006`、`comment.0007`，`blog-li` 重启后保持 active。
